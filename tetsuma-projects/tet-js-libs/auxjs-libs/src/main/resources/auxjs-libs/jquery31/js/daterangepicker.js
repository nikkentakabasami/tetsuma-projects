    ...
    (
        ...
        function(moment, $) {
          var DateRangePicker = function(element, options, cb) {
            ...
            this.clear = false; //if true - value must be cleared
            ...
            // data-api options will be overwritten with custom javascript
            // options
            ...
            if (typeof options.template !== 'string'
              ...
              options.template = '<div class="daterangepicker">'
                  ...
                  + '<button class="clearBtn btn btn-sm btn-default" type="button">Очистить</button> '
            ...
            // if no start/end dates set, check if an input element contains
            // initial values
            ...
            this.container.find('.ranges').on('click.daterangepicker', 'li',
                $.proxy(this.clickRange, this));

            this.container.find('.drp-buttons').on('click.daterangepicker',
                'button.applyBtn', $.proxy(this.clickApply, this))
                .on('click.daterangepicker', 'button.cancelBtn',
                $.proxy(this.clickCancel, this))
                .on('click.daterangepicker', 'button.clearBtn',
                $.proxy(this.clickClear, this));
...
//                'focus.daterangepicker' : $.proxy(this.show, this),	//;!!!  убрал вручную
          ...
          DateRangePicker.prototype = {
            ...
            setEndDate : function(endDate) {
              ...
              if (this.maxSpan
                  && this.startDate.clone().add(this.maxSpan).isBefore(
                      this.endDate))
            ...
            updateView : function() {
              if (this.timePicker) {
                ...
                if (!this.endDate) {
                  this.container.find('.right .calendar-time select').prop(
              ...
              if (this.endDate) {
                ...
                if (!this.singleDatePicker
                    ...
                    && (this.startDate.format('YYYY-MM') == this.leftCalendar.month
                        .format('YYYY-MM') || this.startDate.format('YYYY-MM') == this.rightCalendar.month
                        .format('YYYY-MM'))
                    && (this.endDate.format('YYYY-MM') == this.leftCalendar.month
                        .format('YYYY-MM') || this.endDate.format('YYYY-MM') == this.rightCalendar.month
                        .format('YYYY-MM'))) {
                  return;
                }
                ...
                if (!this.linkedCalendars
                    && (this.endDate.month() != this.startDate.month() || this.endDate
            ...
            updateCalendars : function() {
                ...
                if (this.endDate) {
                  ...
                  if (isNaN(minute)) {
                    minute = parseInt(this.container.find(
                        '.left .minuteselect option:last').val(), 10);
                  }
                  ...
                  if (isNaN(minute)) {
                    minute = parseInt(this.container.find(
                        '.right .minuteselect option:last').val(), 10);
                  }
                  second = this.timePickerSeconds ? parseInt(this.container
              ...
              // highlight any predefined range matching the current start and
              // end dates
            ...
            renderCalendar : function(side) {
              ...
              if (this.showDropdowns) {
                ...
                for (var m = 0; m < 12; m++) {
                  if ((!inMinYear || (minDate && m >= minDate.month()))
                    ...
                    monthHtml += "<option value='" + m + "'"
                        + (m === currentMonth ? " selected='selected'" : "")
                        + " disabled='disabled'>" + this.locale.monthNames[m]
                        + "</option>";
                  }
                }
                monthHtml += "</select>";
              ...
              if (this.endDate == null && this.maxSpan) {
                var maxLimit = this.startDate.clone().add(this.maxSpan).endOf(
                    'day');
              ...
              for (var row = 0; row < 6; row++) {
                ...
                for (var col = 0; col < 7; col++) {
                  ...
                  // don't allow selection of date if a custom function decides
                  // it's invalid
            ...
            renderTimePicker : function(side) {
              ...
              // Don't bother updating the time picker if it's currently
              // disabled
              ...
              if (this.maxSpan
                  && (!this.maxDate || this.startDate.clone().add(this.maxSpan)
              ...
              if (side == 'left') {
                ...
                if (timeSelector.html() != '') {
                  ...
                  selected.hour(!isNaN(selected.hour()) ? selected.hour()
                      : timeSelector.find('.hourselect option:selected').val());
                  selected.minute(!isNaN(selected.minute()) ? selected.minute()
                      : timeSelector.find('.minuteselect option:selected')
                          .val());
                  selected.second(!isNaN(selected.second()) ? selected.second()
              ...
              for (var i = start; i <= end; i++) {
                var i_in_24 = i;
                if (!this.timePicker24Hour)
                  i_in_24 = selected.hour() >= 12 ? (i == 12 ? 12 : i + 12)
                      : (i == 12 ? 0 : i);
                ...
                if (i_in_24 == selected.hour() && !disabled) {
                  html += '<option value="' + i + '" selected="selected">' + i
                      + '</option>';
                } else if (disabled) {
                  html += '<option value="' + i
              ...
              if (!this.timePicker24Hour) {
                ...
                if (minDate
                    && selected.clone().hour(12).minute(0).second(0).isBefore(
                        minDate))
                  am_html = ' disabled="disabled" class="disabled"';

                if (maxDate
                    && selected.clone().hour(0).minute(0).second(0).isAfter(
                        maxDate))
            ...
            updateFormInputs : function() {
              ...
              if (this.singleDatePicker
                  || (this.endDate && (this.startDate.isBefore(this.endDate) || this.startDate
                      .isSame(this.endDate)))) {
            ...
            move : function() {
              ...
              if (this.opens == 'left') {
                ...
                var containerLeft = this.element.offset().left
                    - parentOffset.left + this.element.outerWidth() / 2
                    - containerWidth / 2;
                if (containerLeft < 0) {
                  this.container.css({
                    top : containerTop,
...
//                  dp.setStartDate(moment());
//                  dp.setEndDate(moment());  
//                },500,this);
//              }
            ...
            outsideClick : function(e) {
              ...
              if (
              // ie modal dialog fix
              e.type == "focusin" || target.closest(this.element).length
            ...
            clickRange : function(e) {
              ...
              if (label == this.locale.customRangeLabel) {
                ...
                cal = this.container.find('.drp-calendar.left');

                var selMonth = parseInt(cal.find('.monthselect').val(), 10);
                var selYear = cal.find('.yearselect').val();

                this.relativity = null;
                this.clear = false;

                if (label == "Текущий год") {

                  this.startDate = moment().startOf('year');
                  this.endDate = moment().add(1, 'year').startOf('year');
                  this.clickApply();

                } else if (label == "Текущий квартал") {

                  var currMonth = parseInt(moment().format('MM'), 10);
                  var oddQuartalMonth = (currMonth - 1) % 3;
                  var startOfQuartal = moment().startOf('month').subtract(
                      oddQuartalMonth, 'month');

                  this.startDate = startOfQuartal;
                  this.endDate = startOfQuartal.clone().add(3, 'month');

                  this.clickApply();

                } else if (label == "Текущий месяц") {
                  this.startDate = moment().startOf('month');
                  this.endDate = moment().add(1, 'month').startOf('month');
                  this.clickApply();

                } else if (label == "Месяц по календарю") {

                  this.startDate = moment().set('month', selMonth).set('year',
                      selYear).startOf('month');
                  this.endDate = this.startDate.clone().add(1, 'month');
                  this.clickApply();

                } else if (label == "Год по календарю") {

                  this.startDate = moment().set('month', selMonth).set('year',
                      selYear).startOf('year');
                  this.endDate = this.startDate.clone().add(1, 'year');
                  this.clickApply();

                } else if (label == "I квартал") {
                  var startOfQuartal = moment().set('year', selYear).set(
                      'month', 0).startOf('month');
                  this.startDate = startOfQuartal;
                  this.endDate = startOfQuartal.clone().add(3, 'month');
                  this.clickApply();

                } else if (label == "II квартал") {
                  var startOfQuartal = moment().set('year', selYear).set(
                      'month', 3).startOf('month');
                  this.startDate = startOfQuartal;
                  this.endDate = startOfQuartal.clone().add(3, 'month');
                  this.clickApply();

                } else if (label == "III квартал") {
                  var startOfQuartal = moment().set('year', selYear).set(
                      'month', 6).startOf('month');
                  this.startDate = startOfQuartal;
                  this.endDate = startOfQuartal.clone().add(3, 'month');
                  this.clickApply();

                } else if (label == "IV квартал") {
                  var startOfQuartal = moment().set('year', selYear).set(
                      'month', 9).startOf('month');
                  this.startDate = startOfQuartal;
                  this.endDate = startOfQuartal.clone().add(3, 'month');
                  this.clickApply();

                } else if (label == "До даты") {
                  this.relativity = "<";
            ...
            clickDate : function(e) {
              ...
              // * alternate between selecting a start and end date for the
              // range,
              // * if the time picker is enabled, apply the hour/minute/second
              // from the select boxes to the clicked date
              // * if autoapply is enabled, and an end date was chosen, apply
              // the selection
              ...
              // This is to cancel the blur event handler if the mouse was in
              // one of the inputs
              e.stopPropagation();
              ...
              for ( var range in this.ranges) {
                if (this.timePicker) {
                  var format = this.timePickerSeconds ? "YYYY-MM-DD HH:mm:ss"
                      : "YYYY-MM-DD HH:mm";
                  // ignore times when comparing dates if time picker seconds is
                  // not enabled
                  if (this.startDate.format(format) == this.ranges[range][0]
                    ...
                    this.chosenLabel = this.container.find(
                        '.ranges li:eq(' + i + ')').addClass('active').attr(
                        'data-range-key');
                    break;
                  }
                } else {
              ...
              if (customRange) {
                if (this.showCustomRangeLabel) {
                  this.chosenLabel = this.container.find('.ranges li:last')
                      .addClass('active').attr('data-range-key');
                } else {
            ...
            monthOrYearChanged : function(e) {
              var isLeft = $(e.target).closest('.drp-calendar')
                  .hasClass('left'), leftOrRight = isLeft ? 'left' : 'right', cal = this.container
            ...
            timeChanged : function(e) {
              ...
              if (isNaN(minute)) {
                minute = parseInt(cal.find('.minuteselect option:last').val(),
                    10);
              }
                ...
                if (this.singleDatePicker) {
                  this.endDate = this.startDate.clone();
                    ...
                    && this.endDate.format('YYYY-MM-DD') == start
                        .format('YYYY-MM-DD') && this.endDate.isBefore(start)) {
              ...
              // update the calendars so all clickable dates reflect the new
              // time component
              this.updateCalendars();
