...
import org.eclipse.jetty.server.handler.ResourceHandler.ResourceContext;
 ...
 * 
 */
public class TetJettyServerExperimental {
	...
	ResourceContext resourceHandler;
...
//		webAppContext.setAttribute("org.eclipse.jetty.server.webapp.WebInfIncludeJarPattern", ".*/*.jar$");
		...
		resourceFactory = ResourceFactory.of(server);
...
//		webAppContext.setWarResource(baseResource);
		...
		//дополнительный статический контент
		List<Resource> staticResources = new ArrayList<>();
		for(String as: options.getClasspathStaticContent()) {
			staticResources.add(resourceFactory.newClassLoaderResource(as));
  	}
		
    
		for (Path path : options.getFileSystemStaticContent()) {
			staticResources.add(resourceFactory.newResource(path));
		}
		
		
		
		
		resourceHandler = new ResourceContext();
		resourceHandler.setContextPath(options.getContextPath());
    resourceHandler.setBaseResource(ResourceFactory.combine(staticResources));
...
//    contextsHandlers.addHandler(resourceHandler);
    
    contextsHandlers.setHandlers(new org.eclipse.jetty.server.Handler[] { webAppContext, resourceHandler });
    
    
		
        handlers = new Handler.Sequence();
        handlers.addHandler(contextsHandlers);
